...
public:
  ...
  int height() const;
  int heighta(const Node<T>* node) const;
  int whatlevelamI(const T& val) const;

  std::string ancestors(T) const;
  bool ancestorsa(const Node<T>* node,const T& val, std::stringstream& aux) const;
};
...
template <class T>
int BST<T>::heighta(const Node<T>* node) const {
  if (node == 0) {
    return 0;
  }
  int leftHeight = heighta(node->left);
  int rightHeight = heighta(node->right);
...
template <class T>
bool BST<T>::ancestorsa(const Node<T>* node, const T& target, std::stringstream& result) const {
  if (node == 0) {
    return false;
  }

  if (node->value == target || ancestorsa(node->left, target, result) || ancestorsa(node -> right, target, result)) {
